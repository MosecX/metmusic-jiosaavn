...
/// AddonService — the central addon router for MetMusic.
///
...
class AddonService extends ChangeNotifier {
  ...
  Future<AddonAlbum?> getAlbumDetail(String albumId,
      {String? addonId, int limit = 300}) async {
    final id = addonId ?? _activeAddonId;
    if (id == null) return null;
    final manifest = _getManifest(id);
    if (manifest == null) return null;
  ...
  // ========== Recommendations / Browse / Health ==========

  /// Recommended tracks based on a seed track. Uses the active addon.
  Future<List<Track>> getRecommendations(String seedTrackId,
      {int limit = 24}) async {
    final id = _activeAddonId;
    if (id == null) return [];
    final manifest = _getManifest(id);
    if (manifest == null) return [];
    if (manifest.addonType == AddonType.user) {
      final recs = await _userHandlers[id]?.getRecommendations(seedTrackId,
              limit: limit) ??
          [];
      return recs.map((t) => trackFromAddonTrack(t)).toList();
    }
    try {
      final response = await _dio.get('${manifest.baseUrl}/recommendations',
          queryParameters: {'id': seedTrackId, 'limit': limit});
      final data = _toMap(response.data);
      final items = (data['data']?['items'] ?? data['items']) as List<dynamic>? ??
          [];
      return items
          .map((e) => AddonTrack.fromJson(
              e is Map && e['track'] != null
      ...
      print('[AddonService] getRecommendations error for ${manifest.id}: $e');
      return [];
    }
  }

  /// Resolve the native catalog audio quality for a track id.
  Future<String?> getTrackQuality(String trackId) async {
    final id = _activeAddonId;
    if (id == null) return null;
    final manifest = _getManifest(id);
    if (manifest == null) return null;
    if (manifest.addonType == AddonType.user) {
      return _userHandlers[id]?.getTrackAudioQuality(trackId);
    }
    return null;
  }

  /// Search only albums using the active addon.
  Future<List<AddonAlbum>> searchAlbums(String query, {int? limit}) async {
    final id = _activeAddonId;
    if (id == null) return [];
    final manifest = _getManifest(id);
    if (manifest == null) return [];
    if (manifest.addonType == AddonType.user) {
      return await _userHandlers[id]
              ?.searchAlbums(query, limit: limit ?? 10) ??
          [];
    }
    try {
      final response = await _dio.get('${manifest.baseUrl}/search',
          queryParameters: {'al': query, 'limit': limit ?? 10});
      final data = _toMap(response.data);
      final items =
          (data['data']?['albums']?['items'] ?? data['albums']?['items'])
              as List<dynamic>? ??
          [];
      return items
          .map((a) => AddonAlbum.fromJson(
              Map<String, dynamic>.from(a), addonId: manifest.id))
          .toList();
    } catch (e) {
      print('[AddonService] searchAlbums error for ${manifest.id}: $e');
      return [];
    }
  }

  /// Probe the active addon's API health (online/version/latency).
  Future<ApiHealthStatus?> checkApiHealth() async {
    ...
    if (manifest == null) return null;
    if (manifest.addonType == AddonType.user) {
      return await _userHandlers[id]?.checkHealth();
    }
    try {
      ...
      if (response.statusCode == 200) {
        return ApiHealthStatus(
          online: true,
          version: _toMap(response.data)['version']?.toString(),
      ...
      return const ApiHealthStatus(online: false);
    }
  }
