...
class DashNativeParser {
  ...
  /// Parses an already-fetched MPD XML string (e.g. read from a local file).
  static DashManifest parseContent(String xmlString, {String baseUrl = ''}) {
    ...
    if (baseUrlElements.isNotEmpty) {
      resolvedBaseUrl = baseUrlElements.first.innerText.trim();
    }
    ...
    final initSegmentUrl = _buildUrl(resolvedBaseUrl, initialization);
    ...
    if (segmentTimeline != null) {
      ...
      if (durationStr != null) {
        ...
        if (mpdElement != null) {
          final mediaPresentationDuration =
    ...
    for (int i = 0; i < totalSegments; i++) {
      ...
      mediaSegmentUrls.add(_buildUrl(resolvedBaseUrl, currentMedia));
    }
