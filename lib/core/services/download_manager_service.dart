...
import 'download_manager_native.dart'
    if (dart.library.js_interop) 'download_manager_web.dart' as dm;
...
class DownloadManagerService extends ChangeNotifier {
  ...
  Future<bool> downloadTrack({
    ...
    if (kIsWeb) {
      ...
      notifyListeners();
      try {
        final success = await dm.triggerWebDownload(streamUrl, '$safeName.m4a', track: track);
  ...
  Future<void> _nativeRequestPermissions() async {
    ...
    await dm.requestNativePermissions(
